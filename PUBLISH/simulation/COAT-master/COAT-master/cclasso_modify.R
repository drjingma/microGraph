...
boot_cclasso <- function(x, sigma, lambda2, n_boot = 20, 
  ...
  # Bootstrap procedure
  sam_boot <- matrix(sample(1:n, size = n * n_boot, replace = F), 
                     ncol = n_boot);
  for(k in 1:n_boot) {
    ind_samp <- sam_boot[, k];
