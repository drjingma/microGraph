...
rebacca = function(x, nbootstrap=50, N.cores = 1){
	...
	#replace zeros with minimum*0.5
	x_min = min(x[x!=0])
	x[x==0] = x_min * 0.5
...
# e) rebacca modified Main function written by Yuanpei Cao#
# result is basis covariance matrix and average number of selected variables by LASSO
#  used to calculated rho in equation (11) 
#  input: x: n x p matrix, each row represent different sample, and each column represent different taxa
rebacca_main = function(x, nbootstrap = 1, N.cores = 1){
  startTime <- proc.time()
  res.reb <- rebacca(t(x), nbootstrap = nbootstrap, N.cores = N.cores)
  res.tau <- stability_cutoff(res.reb$Stability, res.reb$q, B = 50, FWER = 0.05)
  reb.adj <- sscore2adjmatrix(res.reb$Stability, res.tau)
  reb.est <- rebacca_adjm2corr(t(x), reb.adj)
  sigma <- reb.est$cov
  corr <- reb.est$corr
