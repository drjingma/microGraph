...
sparcc <- function(x, max.iter=20, th=0.1, exiter=10, countStatus = TRUE){
  xdim <- dim(x)
  ...
  for (i in 1:max.iter){
#    cat("Iteration: %d\n",i)
    tmpres <- compute.corr(x, iter=exiter, th=th, countStatus = countStatus)
...
compute.corr <- function(x, iter=10, th=0.1, countStatus = TRUE){
